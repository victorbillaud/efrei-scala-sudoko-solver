...
class SudokuSolverTests extends munit.FunSuite {
  val sampleGrid: SudokuSolver.Grid = List(
  ...
  test("isValid should work correctly") {
    assert(SudokuSolver.isValid(sampleGrid, 0, 2, 1)) // this should pass
    assert(!SudokuSolver.isValid(sampleGrid, 0, 2, 3)) // this should pass
  }
}
